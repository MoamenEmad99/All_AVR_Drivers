...
static ADC_callback_t ADC_callback_ptr = NULL;
...
void ADC_init(uint8 ADC_CH){
	...
	else if(ADC_conf.Interrupt == INTERRUPT_ENABLE){
		setbit(SREG,GIE);
...
void ADC_start_conversion(uint8 ADC_CH){
	//selecting ADC channel
	ADMUX = (ADMUX & 0xE0) | ADC_CH;
...
void ADC_set_callback(ADC_callback_t ADC_callback){
	ADC_callback_ptr = ADC_callback;
}

ISR(ADC_INT){
	if(ADC_conf.Adjust == LEFT) ADC_callback_ptr((ADC>>6));
	else if(ADC_conf.Adjust == RIGHT) ADC_callback_ptr(ADC);	
}