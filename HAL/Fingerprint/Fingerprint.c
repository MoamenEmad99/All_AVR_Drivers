...
const uint8 f_search[]={0xEF, 0x1, 0xFF, 0xFF, 0xFF, 0xFF, 0x1, 0x0, 0x8, 0x4, 0x1, 0x0, 0x1, 0x0, 0xD, 0x0, 0x1C};
...
void fingerprint_init(uint8 *FP_flag){
	...
	*FP_state = NO_FINGER;
	...
	if(FP_INT == FP_EN){
		external_interrupt_register_callback(FP_ISR, NULL, NULL);
		EXT_INT_Init();
		EXT_INT_SET_CONFIG();
	}
...
uint8 FP_sendCommand(uint8 *pack, uint16 len){
	...
	TIMER0_DelayMilliSeconds_with_Blocking(100);
		...
		if((FP_buffer[0] == 0xEF) && (FP_buffer[1] == 0x01)){
			...
			if(FP_buffer[6] == 0x07){						
				...
				if(FP_buffer[9] == 0){		
					...
					for(uint8 i=0; i<package_content_len-2; i++)
	...
	if(FP_sendCommand(f_detect, SIZE_OF_DETECT) == PASS){
		...
		if(FP_sendCommand(f_img2ch1, SIZE_OF_IMG_TO_CH) == PASS){
			...
			if (FP_sendCommand(f_search, SIZE_OF_SEARCH) == PASS){
				...
				uint8 scannedID = package_content[0];
				scannedID<<8;
